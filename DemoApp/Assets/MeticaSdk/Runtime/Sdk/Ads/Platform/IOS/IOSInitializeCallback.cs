...
namespace Metica.Ads.IOS
{
    ...
    public class IOSInitializeCallback
    {
        ...
        private static extern void ios_sdkInitialize(string apiKey, string appId, string userId,
            string mediationInfoKey, OnInitializeSuccessDelegate result);
        ...
        private static void OnInitialized(string meticaAdsInitializationResponseJson)
        {
            ...
            if (tcs == null)
            {
                MeticaAds.Log.LogError(() =>
                    $"{TAG} IOSInitializeCallback OnInitialized called but _currentTcs is null");
                return;
            }
            ...
            try
            {
                MeticaAds.Log.LogDebug(() =>
                    $"{TAG} IOSInitializeCallback OnInitialized received JSON: {meticaAdsInitializationResponseJson}");

                var meticaInitResponse = MeticaInitResponse.FromJson(meticaAdsInitializationResponseJson);
                if (meticaInitResponse == null)
                {
                    MeticaAds.Log.LogError(() =>
                        $"{TAG} IOSInitializeCallback OnInitialized failed to parse JSON: {meticaAdsInitializationResponseJson}");
                    _currentTcs = null;
                    tcs.SetException(new Exception(
                        $"Failed to parse initialization response JSON: {meticaAdsInitializationResponseJson}"));
                    return;
                }
                ...
                {
                    MeticaAds.Log.LogDebug(() =>
                        $"{TAG} IOSInitializeCallback OnInitialized SmartFloors is null after parsing, attempting manual parse");
                    ...
                    try
                    {
                        ...
                        var rootWrapper =
                            JsonUtility.FromJson<MeticaInitResponseJsonWrapper>(meticaAdsInitializationResponseJson);
                        ...
                        {
                            MeticaAds.Log.LogError(() =>
                                $"{TAG} IOSInitializeCallback could not find userGroup type in object");
                            _currentTcs = null;
                            tcs.SetException(new Exception(
                                $"Unknown userGroup structure in JSON. Expected type, got: {meticaAdsInitializationResponseJson}"));
                            return;
                        }
                        ...
                        MeticaAds.Log.LogDebug(() =>
                            $"{TAG} IOSInitializeCallback manually parsed SmartFloors: userGroup={userGroup}, isSuccess={isSuccess}");
                    }
                    catch (Exception parseEx)
                    {
                        MeticaAds.Log.LogError(() =>
                            $"{TAG} IOSInitializeCallback failed to manually parse SmartFloors: {parseEx.Message}\nStack: {parseEx.StackTrace}");
                        _currentTcs = null;
                        tcs.SetException(new Exception(
                            $"Failed to parse SmartFloors from JSON: {meticaAdsInitializationResponseJson}", parseEx));
                        return;
                    }
                }
            ...
            {
                MeticaAds.Log.LogError(() =>
                    $"{TAG} IOSInitializeCallback OnInitialized exception: {ex.Message}\nStack: {ex.StackTrace}");
                _currentTcs = null;
                tcs.SetException(ex);
            }
        }
